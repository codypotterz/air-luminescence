//
// ********************************************************************
// * License and Disclaimer                                           *
// *                                                                  *
// * The  Geant4 software  is  copyright of the Copyright Holders  of *
// * the Geant4 Collaboration.  It is provided  under  the terms  and *
// * conditions of the Geant4 Software License,  included in the file *
// * LICENSE and available at  http://cern.ch/geant4/license .  These *
// * include a list of copyright holders.                             *
// *                                                                  *
// * Neither the authors of this software system, nor their employing *
// * institutes,nor the agencies providing financial support for this *
// * work  make  any representation or  warranty, express or implied, *
// * regarding  this  software system or assume any liability for its *
// * use.  Please see the license in the file  LICENSE  and URL above *
// * for the full disclaimer and the limitation of liability.         *
// *                                                                  *
// * This  code  implementation is the result of  the  scientific and *
// * technical work of the GEANT4 collaboration.                      *
// * By using,  copying,  modifying or  distributing the software (or *
// * any work based  on the software)  you  agree  to acknowledge its *
// * use  in  resulting  scientific  publications,  and indicate your *
// * acceptance of all terms of the Geant4 Software license.          *
// ********************************************************************
//
/// \file OpNovice/include/OpNoviceRun.hh
/// \brief Definition of the OpNoviceRun class
//
//
//
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

#ifndef OpNoviceRun_h
#define OpNoviceRun_h 1

#include "G4Run.hh"
#include "globals.hh"
#include "G4ParticleDefinition.hh"

#include <array>

const G4int kAbs = 0;
const G4int kDim = 1;
class G4ParticleDefinition;

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

class OpNoviceRun : public G4Run
{
 public:
  OpNoviceRun();
  ~OpNoviceRun();
  
  void Add(G4int id, G4double de, G4double dl);
  void FillPerEvent();
  void Reset();
  
  G4String  GetVolumeName(G4int id) const;
  G4double  GetEdep(G4int id) const;
  G4double  GetTrackLength(G4int id) const;
  

  //void SetPrimary(G4ParticleDefinition* particle, G4double energy);
  void SetPrimary(G4ParticleDefinition* particle);

  //functions for radioluminescence spectrum
  void AddScintillationEnergy(G4double en) { fScintEnergy += en; }
  void AddScintillationNo(void) {fScintCount += 1;}

  // function for average no of photons per event
  void AddScintillation(G4double n)
  {
    fScintillationCounter += n;
    fScintillation2 += n * n;
  };
  

  void Merge(const G4Run*) override;
  void EndOfRun();

 private:
     
  G4ParticleDefinition* fParticle;
  G4double fEnergy;
     
  std::array<G4String, kDim>  fVolumeNames;
  std::array<G4double, kDim>  fEdep;
  std::array<G4double, kDim>  fTrackLength;    
     
  G4double fScintillationCounter;
  G4double fScintillation2;
  
  G4double fScintEnergy;
  G4double fScintCount;

};

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// inline functions

inline void OpNoviceRun::Add(G4int id, G4double de, G4double dl) {
  fEdep[id] += de; 
  fTrackLength[id] += dl;
}

inline G4String  OpNoviceRun::GetVolumeName(G4int id) const {
  return fVolumeNames[id];
}

inline G4double  OpNoviceRun::GetEdep(G4int id) const {
  return fEdep[id];
}   

inline G4double  OpNoviceRun::GetTrackLength(G4int id) const {
  return fTrackLength[id];
}

#endif
