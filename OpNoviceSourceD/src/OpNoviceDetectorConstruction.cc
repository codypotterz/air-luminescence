  ...
  : G4VUserDetectorConstruction()
{
  ...
  Ag_x = 15.0 * mm;
  Ag_y = 50.0 * mm;
  Ag_z = 0.125 * mm; // Silver Plate (30x100x0.25)mm

  Au_x = 10.0 * mm;
  Au_y = 50.0 * mm;
  Au_z = 0.001 * mm; // Gold cover = (20x100x0.002)mm
...
// Air Gold and Silver
  ...
  G4Material* air        = man->ConstructNewGasMaterial("Air", "G4_AIR", 295.15*kelvin, 1.0*atmosphere);
  G4Material* gold       = man->FindOrBuildMaterial("G4_Au");
  G4Material* silver     = man->FindOrBuildMaterial("G4_Ag");
...
// The experimental Hall = world volume
//
...
//The Gold Cover
//
    G4Box* GoldCover = new G4Box("GoldCover", Au_x, Au_y, Au_z);

    G4LogicalVolume* GoldCover_log =
     new G4LogicalVolume(GoldCover, gold, "GoldCover", 0, 0, 0);

    GoldCover_phys =
     new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, -0.0015*mm), GoldCover_log, "GoldCover", Absorber_log, false, 0);
